...
> 调研日期：2026-07-19
> 调研目标：将 AgentBuddy 自定义 plugin.yaml 格式对齐业界主流规范（Claude Code / Codex CLI / OpenCode / mem0 / VSCode / npm）
...
**关键行为：未识别字段会被忽略**（仅 `claude plugin validate --strict` 才警告），因此 AgentBuddy 扩展字段 `llm/subagents/rules/commands/envVars/scripts` 在跨 IDE 导出时安全。
...
**对 AgentBuddy 的启示：** OpenCode 不是清单式，导出到 OpenCode 时只能转换为 npm 包或本地 `.ts` 文件，不能直接复用 plugin.yaml。当前策略：导出 skill/MCP/agent 的独立文件到 `~/.config/opencode/` 对应目录即可，不走 plugin 机制。
...
**对 AgentBuddy 的启示：** 这是 `mcpServers + hooks + skills` 组合分发的标准范例，验证了 AgentBuddy 把三者放在同一 plugin.yaml 的设计可行。
...
AgentBuddy 的 `scripts` 字段对齐上述生命周期，**旧字段 `init` 等价于 `postinstall`**（向后兼容读取）。

## 8. AgentBuddy 对齐方案

### 8.1 字段对齐矩阵

| AgentBuddy 字段 | 类型 | 来源 | 对齐说明 |
|---|---|---|---|
...
| `manifest_version` | string | AgentBuddy | 清单版本（新增，未来 schema 升级用） |
...
| `subagents` | array | AgentBuddy 扩展 | subagent 角色名（Claude 忽略未识别字段，安全） |
| `rules` | array | AgentBuddy 扩展 | rules 文件路径（同上） |
| `llm` | array | AgentBuddy 扩展 | LLM Provider 配置（同上） |
| `envVars` | object | AgentBuddy 扩展 | 环境变量（同上） |
